...
<div class="container">
    <div class="card card-custom">
        <div class="card-header">
            <h3 class="card-title">Edit Tasheer Appointment #TSH-{{ $appointment->id }}</h3>
        </div>
        <form method="POST" action="{{ route('admin.tasheer.appointments.update', $appointment->id) }}">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-12 text-center mb-4">
                        ...
                        <div class="mt-2">
                            <a href="{{ asset('uploads/' . $appointment->passport_pic) }}" download="passport_{{ $appointment->id }}" class="btn btn-sm btn-light-primary font-weight-bold">
                                <i class="fas fa-download"></i> Download Passport
                            </a>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body text-center">
                                    <img src="{{ asset('uploads/' . $appointment->payment->proof_image) }}" style="width: 200px; height: auto; border-radius: 5px; box-shadow: 0 0 20px rgba(0,0,0,0.1);">
                                </div>
                                <div class="modal-footer">                                    
                                    <a href="{{ asset('uploads/' . $appointment->payment->proof_image) }}" target="_blank" class="btn btn-primary font-weight-bold">Open in New Tab</a>
            ...
            <div class="card-footer">
                ...
                <a href="{{ route('admin.tasheer.appointments') }}" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        // Handle image preview for documents
        $('.view-image').on('click', function() {
            var imageSrc = $(this).attr('data-src');
            $('#modalImg').attr('src', imageSrc);
        });

        // Clear the source when modal is closed
